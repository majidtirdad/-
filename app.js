...
/* ══ PROJECTS ══ */
const DEFAULTS = [
  {id:1, en:'Sadrazadeh', fa:'صدرازاده', catEn:'Commercial',    catFa:'تبلیغاتی',          descEn:'A premium brand campaign.',   descFa:'یک کمپین برند پریمیوم.',      lnk:'https://instagram.com', img:'portrait.jpg'},
  {id:2, en:'Amol Cable',  fa:'کابل آمل', catEn:'Brand Film',    catFa:'فیلم برند',          descEn:'Industrial storytelling.',    descFa:'داستان‌پردازی صنعتی.',         lnk:'https://instagram.com', img:'portrait.jpg'},
  {id:3, en:'Mey Music',   fa:'می موزیک', catEn:'Music Video',   catFa:'موزیک ویدیو',        descEn:'Cinematic music visuals.',    descFa:'ویژوال موزیک سینماتیک.',      lnk:'https://instagram.com', img:'portrait.jpg'},
  {id:4, en:'Cafe Malt',   fa:'کافه مالت',catEn:'Social Content',catFa:'محتوای شبکه اجتماعی',descEn:'Lifestyle & brand identity.', descFa:'لایف‌استایل و هویت برند.',    lnk:'https://instagram.com', img:'portrait.jpg'},
  {id:5, en:'Lamari',      fa:'لاماری',   catEn:'Fashion Film',  catFa:'فیلم فشن',           descEn:'Elegant fashion visuals.',    descFa:'ویژوال‌های زیبای فشن.',        lnk:'https://instagram.com', img:'portrait.jpg'},
  {id:6, en:'Yekta Home',  fa:'یکتا هوم', catEn:'Real Estate',   catFa:'مسکن',               descEn:'Premium property showcase.',  descFa:'نمایش ملک پریمیوم.',          lnk:'https://instagram.com', img:'portrait.jpg'},
];
function getP()  { try { const s=localStorage.getItem('mt_p'); return s?JSON.parse(s):DEFAULTS; } catch { return DEFAULTS; } }
function setP(a) { try { localStorage.setItem('mt_p', JSON.stringify(a)); } catch {} }
...
function saveProj() {
  const en=document.getElementById('ap-en').value.trim();
  const fa=document.getElementById('ap-fa').value.trim();
  const catEn=document.getElementById('ap-cat-en').value.trim();
  const catFa=document.getElementById('ap-cat-fa').value.trim();
  const descEn=document.getElementById('ap-desc-en').value.trim();
  const descFa=document.getElementById('ap-desc-fa').value.trim();
  const lnk=document.getElementById('ap-lnk').value.trim();
  const fi=document.getElementById('ap-img');
  const editId=document.getElementById('edit-id').value;
  const msg=document.getElementById('addMsg');
  if (!en||!fa||!lnk) { msg.style.color='#ef4444'; msg.textContent='Fill required fields.'; return; }
  const finish = img => {
    let arr = getP();
    if (editId) {
      const idx = arr.findIndex(p=>String(p.id)===editId);
      if (idx>-1) arr[idx] = {...arr[idx], en,fa,catEn,catFa,descEn,descFa,lnk, img:img||arr[idx].img};
    } else {
      arr.push({id:Date.now(), en,fa,catEn,catFa,descEn,descFa,lnk, img:img||'portrait.jpg'});
    }
    setP(arr); renderP(); renderAdmList(); resetForm();
    msg.style.color='#10b981'; msg.textContent=editId?'✓ Updated!':'✓ Added!';
    setTimeout(()=>msg.textContent='', 3000);
  };
  if (fi.files[0]) { const r=new FileReader(); r.onload=e=>finish(e.target.result); r.readAsDataURL(fi.files[0]); }
  else finish(null);
}
function resetForm() {
